...
class Connector:
    ...
    async def _send_game_info_to_players(self):
        ...
        await i_s._send_game_creation_info_to_opp(self._game.id)
    ...
    async def _set_opponent(self):
        if self._get_conflict_side() == "teacher":
            self._opp = await get_longest_waiting_player("student")
        else:
            self._opp = await get_longest_waiting_player("teacher")

    async def _create_game(self):
        game = await create_game(self._consumer.get_game_user(), self._opp)
