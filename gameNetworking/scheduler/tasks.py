...
async def limit_hub_time(game_user_id):
...
async def limit_action_time(game_user_id):
    game_user = await get_game_user(game_user_id)
    if game_user:
        channel_layer = get_channel_layer()
        await channel_layer.send(
...
async def limit_reaction_time(game_user_id):
    game_user = await get_game_user(game_user_id)
    if game_user:
        channel_layer = get_channel_layer()
        await channel_layer.send(
...
async def limit_opponent_rejoin_time(game_user_id, game_id):
    game_user = await get_game_user(game_user_id)
    if game_user:
        channel_layer = get_channel_layer()
        await channel_layer.send(
