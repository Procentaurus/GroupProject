...
async def perform_cleanup_impl(consumer): # is called after game's end, when the end was triggered by the opponent or from standard cleanup procedure
    ...
    if consumer.game_id is None: # block used when opponents consumer already deleted game and game users' data
        flag = await delete_game_user(consumer.game_user_id)
        if not flag:
        ...
        if game is not None:
            ...
            teacher_player, student_player = await get_both_players_from_game(consumer.game_id)
            ...
            flag3 = await delete_game(consumer.game_id)
...
async def disconnect_impl(consumer):
    if consumer.closure_from_user_side:  # disconnnect from user side
        if consumer.game_id is not None:
            await consumer.send_message_to_opponent(None, "game_end")
        await consumer.perform_cleanup()
    if consumer.game_id is not None:
        await consumer.channel_layer.group_discard(f"game_{consumer.game_id}", consumer.channel_name)