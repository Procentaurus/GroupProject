...
class GameUser(models.Model): # user has new instance of GameUser created for every new game
    ...
    state = models.CharField(choices=PLAYER_STATES, default='in_hub', null=False, blank=False)
    ...
    action_cards_in_shop = models.ManyToManyField(ActionCard, related_name="action_cards_in_shop")
    ...
    def check_if_have_reaction_card_in_shop(self, reaction_card_id, amount):
        result = check_if_have_reaction_card_in_shop_impl(self, reaction_card_id, amount)
        return result
...
class GameArchive(models.Model):
    ...
    teacher_player = models.OneToOneField(MyUser, related_name="teacher_player", on_delete=models.SET_NULL, null=True)
    student_player = models.OneToOneField(MyUser, related_name="student_player", on_delete=models.SET_NULL, null=True)
